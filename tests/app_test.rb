...
class ApplicationTest < MiniTest::Unit::TestCase
  ...
  def retrieve_all_notes()
    ...
    returned_notes = JSON.parse(last_response.body)
    refute_nil returned_notes
    returned_notes
  end
  ...
  def test_add_and_retrieve_all_notes
    # Create a new note, then retrieve it and
    # assert that we get the same stuff back!

    note = {
      "subject" => "Don't forget",
      "content" => "Test add and retrieve notes!"
    }

    note_id = make_a_note(note)

    # Retrieve the all the notes
    notes = retrieve_all_notes()
    # notes should not be nill
    refute_nil notes
    #get the last note, which should be the one we just added
    last_note = notes.last

    note.each_key do |k|
      assert_equal note[k], last_note[k]
    end
  end
